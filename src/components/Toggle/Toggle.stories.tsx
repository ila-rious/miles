...
export const Playground: Story = {
  name: 'Playground',
  parameters: {
    docs: { description: { story: 'All controls live. Flip it on and off, change colour, size, label position.' } },
  },
  render: (args) => {
    const [checked, setChecked] = React.useState(args.checked ?? false);
    React.useEffect(() => { setChecked(args.checked ?? false); }, [args.checked]);
    const snippet = `<Toggle size="${args.size}" color="${args.color}" labelAlign="${args.labelAlign}"${checked ? ' checked' : ''}${args.disabled ? ' disabled' : ''}${args.label ? ` label="${args.label}"` : ''}${args.showInfoIcon ? ' showInfoIcon' : ''} />`;
    return (
